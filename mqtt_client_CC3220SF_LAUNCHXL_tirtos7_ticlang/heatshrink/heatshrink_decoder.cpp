...
typedef struct {
    ...
    size_t *sizeDataOut;        /* bytes pushed to buffer, so far */
} output_info;
...
HSD_poll_res heatshrink_decoder_poll(heatshrink_decoder *hsd,
        uint8_t *out_buf, size_t out_buf_size, size_t *sizeDataOut) {
    if ((hsd == NULL) || (out_buf == NULL) || (sizeDataOut == NULL)) {
    ...
    *sizeDataOut = 0;
    ...
    while (1) {
        ...
        if (hsd->state == in_state) {
            if (*sizeDataOut == out_buf_size) { return HSDR_POLL_MORE; }
...
static HSD_state st_yield_backref(heatshrink_decoder *hsd,
        output_info *oi) {
    size_t count = oi->buf_size - *oi->sizeDataOut;
    if (count > 0) {
        size_t i = 0;
